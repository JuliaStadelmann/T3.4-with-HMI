...
class CommNet(MLP):
    ...
    def _get_agent_mask(self, batch_size: int, info: dict) -> Tuple[int, Tensor]:
        ...
        if 'alive_mask' in info:
            agent_mask: Tensor = torch.from_numpy(info['alive_mask']) if isinstance(info['alive_mask'], np.ndarray) else info['alive_mask'] his up
        ...
        agent_mask = agent_mask * agent_mask_transpose     
